...
#include <unistd.h>
#include <sys/errno.h>
#include <poll.h>       // 添加这行 - 用于 poll 和 struct pollfd
#define ERR_EXIT(m) \
do \
{   \
    perror(m);  \
...
int main(void) {
    ...
    struct pollfd client[2048]; //存放已连接套接字


    int maxi = 0;

    for (int i = 0; i < 2048; i++) {
        client[i].fd = -1; //初始化为-1,表示空闲
    }
    ...
    client[0].events = POLL_IN; //监听可读事件
